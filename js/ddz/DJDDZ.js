...
DJDDZ.GrabTheLandlord = function () {
    ...
    if (GMain.MaxScore == 3 || (GMain.MaxScore > 0 && GMain.GrabTime == 3)) {
        //地主已产生
        GMain.DealerNum = GMain.LandlordNum; //从地主开始发牌
        GMain.LastHandNum = 0;
        GMain.PokerPanel0.hidePoker = false; //显示底牌
...
DJDDZ.GetPokerType = function (__pokerNumbers, chaiNum) {
    //获取__pokerNumbers的牌型，__pokerNumbers已排序，从小到大
    if (chaiNum == null) chaiNum = 3;
    ...
    if (DJDDZ.CheckPlayPoker(_pokerNumbers)) {
        ...
        if (GMain.LastHandNum == 0) {
            ...
            if (SPN.length == 0) {
                ...
                if (
                    (splitPoker["11"].length > 0 && splitPoker["1"].length > 0 && (splitPoker["11"][0] < splitPoker["1"][0] || nn[1])) ||
            ...
            if (GMain.LastHandPokerType.type != "12") {
                if (GMain.LandlordNum == GMain.DealerNum || GMain.LastHandNum == GMain.LandlordNum) {
                    ...
                    if (GMain.Poker[GMain.LastHandNum].length > 5) {
                        if (
                            (GMain.LastHandPokerType.type == "1" && GMain.LastHandPokerType.length == 1) ||
